        ...
        <div className="w-full max-w-xl sm:max-w-2xl mx-auto flex flex-col gap-3.5 sm:gap-4 px-3 sm:px-4">
          ...
          {activeTab === "Invoices" && (() => {
            ...
            const filteredInvoices = invoicesInYear.filter(
              (invoice) => selectedMonth === "all" || getDocMonth(invoice) === Number(selectedMonth)
            )
                ...
                {loadingDocs ? (
                  ...
                  <div className="py-12 text-center text-slate-400 font-medium text-sm bg-white rounded-2xl border border-slate-200 p-6">
                    {invoices.length === 0
                      ...
                      : selectedMonth !== "all"
                      ? "No tienes facturas en el mes seleccionado."
          ...
          {/* Pestaña: ALBARANES */}
          {activeTab === "DeliveryNotes" && (() => {
            const deliveryNotesInYear = deliveryNotes.filter(
              (deliveryNote) => selectedYear === "all" || getDocYear(deliveryNote) === Number(selectedYear)
            )
            const availableDnMonths = Array.from(
              new Set(deliveryNotesInYear.map((dn) => getDocMonth(dn)).filter((m) => m !== null))
            )
            const filteredDeliveryNotes = deliveryNotesInYear.filter(
              (deliveryNote) => selectedMonth === "all" || getDocMonth(deliveryNote) === Number(selectedMonth)
            )
            ...
            return (
              <div className="w-full flex flex-col gap-3">
                ...
                <MonthFilter
                  selectedMonth={selectedMonth}
                  onSelectMonth={setSelectedMonth}
                  availableMonths={availableDnMonths}
                />
                ...
                {loadingDocs ? (
                  ...
                  <div className="py-12 text-center text-slate-400 font-medium text-sm bg-white rounded-2xl border border-slate-200 p-6">
                    {deliveryNotes.length === 0
                      ...
                      : selectedMonth !== "all"
                      ? "No tienes albaranes en el mes seleccionado."
